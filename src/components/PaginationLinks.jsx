...
export default function PaginationLinks({meta, onPageClick}) {
  ...
  return (
    <div className="flex items-center justify-between border-t border-gray-200 bg-white
        px-4 py-3 sm:px-6 shadow-md mt-4">
      {meta.links && <div className="flex flex-1 justify-between sm:hidden">
        <a
          href="#"
          onClick={(ev) => onClick(ev, meta.links[0])}
          className={
            "relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
            + (!(meta.links[0]['url']) ? 'pointer-events-none opacity-50 ' : '')
          }
        >
          Previous
        </a>
        <a
          ...
          className={
            ...
            + (!(meta.links[meta.links.length - 1]['url']) ? 'pointer-events-none opacity-50 ' : '')
          }
        >
          Next
        </a>
          ...
          <nav className="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
            {meta.links && meta.links.map((link, index) => (
                <a
                    ...
                    aria-current="page"
                    className={
                        "relative z-10 inline-flex items-center border px-4 py-2 text-sm font-semibold focus:z-20 hover:bg-gray-50 "
